...
The authentication flow now includes intelligent user detection and password recovery:
...
graph TD
    ...
    C --> E[Email Entry]
    E --> F[User Lookup]
    F -->|User Found| G[Sign In Screen]
    F -->|New User| H[Password Creation]
    G --> I[Password Entry]
    I --> J[Forgot Password?]
    J -->|Yes| K[Password Reset Flow]
    J -->|No| L[AWS Cognito Auth]
    K --> M[Email Code Entry]
    M --> N[New Password]
    N --> L
    L --> O{Auth Success?}
    O -->|No| I
    O -->|Yes| D
    H --> P[Account Creation]
    P --> Q[Email Verification]
    D -->|No| Q
    D -->|Yes| R{Profile Complete?}
    Q --> S[Resend/Confirm Email]
    S --> R
    R -->|No| T[Profile Creation]
    R -->|Yes| U{Baseline Complete?}
    T --> V[Auto-assign University]
    V --> U
    U -->|No| W[Baseline Welcome]
    U -->|Yes| X[Dashboard]
    W --> Y[Baseline Assessment]
    Y --> Z[Post-Baseline Dashboard]
```

**Key Features:**

1. **User Detection**: After name and email entry, the system checks the `profiles` table for matching name and email. If found, routes to sign-in; if not, continues to password creation.

2. **Forgot Password**: Sign-in screen includes "Forgot Password?" link that initiates a 3-step password reset flow:
   - Step 1: Enter email to receive reset code
   - Step 2: Enter code and new password
   - Step 3: Success confirmation and auto-navigation back to sign-in

3. **Pre-filled Email**: When routing from registration to sign-in (user exists), the email field is automatically pre-filled and read-only.
...
See [Assessment Engine documentation](/assessment) and [API Documentation](/api-reference/api-documentation) for complete details.
