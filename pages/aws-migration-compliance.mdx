...
Student Data Access:
  ...
  University Staff:
    ...
    - Scope: Aggregated data, anonymized individual data
  ...
  private async identifyBottlenecks(metrics: PerformanceMetric[]): Promise<Bottleneck[]> {
    ...
    if (this.averageResponseTime(dbMetrics) > 100) {
      bottlenecks.push({
        ...
        recommendations: ['Optimize queries', 'Add indexes', 'Scale up capacity']
      });
    }
    ...
    if (coldStartRate > 0.1) {
      bottlenecks.push({
        ...
        recommendations: ['Increase provisioned concurrency', 'Optimize function size']
      });
    }
...
✅ **Cost Optimization**: Pay-per-use model with 40% cost reduction  
