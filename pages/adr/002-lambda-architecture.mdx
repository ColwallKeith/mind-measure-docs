...
2. **Memory Limits**: Optimise for cost while maintaining performance
...
- **Cost Optimisation**: Pay only for actual execution time
...
- **Specialized Resources**: Each function optimised for its specific task
...
- **X-Ray**: Distributed tracing for performance optimisation
...
### Function Sizing
```yaml
# Optimised memory allocation based on profiling
analyze-audio:
...
4. **Performance Optimisation**: Continuous optimisation based on metrics
